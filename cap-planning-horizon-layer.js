(function(){
  ...
  function host(){let el=qs('#capPlanningHorizon');if(el)return el;const parent=[qs('#dashboard'),qs('[data-page="dashboard"]'),qs('.main')].find(Boolean);if(!parent)return null;el=document.createElement('section');el.id='capPlanningHorizon';el.className='card planning-horizon';parent.appendChild(el);return el;}
  ...
  function sourceLabel(h){const mode=String(h?.source_quality?.mode||'BACKEND_LIVE').toUpperCase();return mode.includes('LOCAL_FALLBACK')?'MODALITÀ DEGRADATA · FALLBACK LOCALE':'BACKEND LIVE · DATI CONFERMATI';}
  function render(h){
    const el=host();if(!el)return;const t=h.totals||{};const acts=H.prioritizedActions(h).slice(0,6);
    el.innerHTML=`<div class="planning-head"><div><h3>Planning Horizon · 7 giorni</h3><div class="planning-source">${esc(sourceLabel(h))} · Solo dati reali · nessuna disponibilità inferita</div></div><span class="planning-state ${esc(h.state)}">${esc(h.state)}</span></div>
      ...
      <div class="planning-source">Driver app: temperatura e km partenza/arrivo sono dati esterni. CAP li visualizza solo se sincronizzati/importati e non li richiede una seconda volta all’ufficio.</div>`;
  }
  async function load(){
    try{const remote=await api('/api/planning-horizon?days=7');if(remote?.calendar){remote.source_quality={...(remote.source_quality||{}),mode:'BACKEND_LIVE'};window.CapLiveStatus?.report?.('planning',true);render(remote);window.dispatchEvent(new CustomEvent('cap:planning-horizon',{detail:remote}));return remote;}}catch(e){window.CapLiveStatus?.report?.('planning',false,e?.message||'planning backend non disponibile')}
  ...
  async function sync(entries,source='planning-ui'){const clean=H.sanitizeEntries(entries);if(!clean.length)return {ok:true,accepted:0,ignored:0};const payload={source,entries:clean.map(x=>({...x,service_date:x.date}))};const out=await api('/api/planning-horizon/import',{method:'POST',body:JSON.stringify(payload)});await load();return out;}
