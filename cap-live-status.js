...
const HEALTH_CACHE_TTL=15*60*1000;
...
function snapshot(){
  ...
  let state='CHECK',label='Backend non interrogato',detail='CAP resta disponibile; verifica il backend solo quando serve.';
  if(offline){state='OFFLINE';label='Offline';detail='Nessuna rete: usa i dati locali disponibili.';}
  else if(health.ok===false){state='DOWN';label='Backend non disponibile';detail='Modalità degradata: CAP resta aperto senza polling automatico.';}
...
function render(){
  ...
  el.innerHTML=`<span class="cap-live-dot"></span><div class="cap-live-copy"><b>${esc(s.label)}</b><small>${esc(s.detail)}${failed}</small></div><button type="button" data-cap-live-retry ${checking?'disabled':''}>${checking?'VERIFICA…':'VERIFICA'}</button>`;
}
...
async function fetchHealth(path){
  const base=apiBase();if(!base)throw new Error('API base assente');const ctrl=new AbortController(),timeout=setTimeout(()=>ctrl.abort(),5000),started=Date.now();
...
async function check(force=false){
  ...
  if(navigator.onLine===false){render();return snapshot();}
  if(!force&&health.checkedAt&&Date.now()-health.checkedAt<HEALTH_CACHE_TTL)return snapshot();
