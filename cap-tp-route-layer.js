...
function publishProfiles(){window.capTpProfiles=profiles;window.dispatchEvent(new CustomEvent('cap-tp-profiles-updated',{detail:{count:profiles.size}}));}
...
function evidenceValue(v,suffix=''){return v===null||v===undefined||v===''?'—':`${esc(v)}${suffix}`}
function evidenceBox(x){
  if(!x||x.state==='NOT_SYNCED'||!x.evidence)return `<div class="captp-driver-evidence"><div class="captp-kicker">APP AUTISTI · EVIDENZA ESTERNA</div><div class="captp-data-gap">Non sincronizzato. Temperatura e km non vanno reinseriti manualmente in CAP.</div></div>`;
  const e=x.evidence,c=x.completeness||{};
  const km=(e.departure_odometer_km!=null&&e.return_odometer_km!=null)?Math.max(0,Number(e.return_odometer_km)-Number(e.departure_odometer_km)):null;
  const tempStatus=String(e.temperature_status||'UNKNOWN').toUpperCase();
  return `<div class="captp-driver-evidence"><div class="captp-kicker">APP AUTISTI · EVIDENZA ESTERNA</div><div class="captp-stats"><span class="captp-stat">Temp part. ${evidenceValue(e.departure_temperature_c,' °C')}</span><span class="captp-stat">Temp rientro ${evidenceValue(e.return_temperature_c,' °C')}</span><span class="captp-stat">Km part. ${evidenceValue(e.departure_odometer_km)}</span><span class="captp-stat">Km rientro ${evidenceValue(e.return_odometer_km)}</span>${km!=null?`<span class="captp-stat">Percorsi ${esc(km)} km</span>`:''}</div><div class="captp-meta">Fonte: ${esc(e.source_system||'esterna')} · Sync: ${esc(e.synced_at?new Date(e.synced_at).toLocaleString('it-IT'):'—')} · Completezza ${esc(c.score??0)}% · Temperatura: ${esc(tempStatus)}</div></div>`;
}
async function loadDriverEvidence(routeId){
  const box=document.getElementById('captp-driver-evidence-host');if(!box)return;
  try{const x=await api(`/api/routes/${encodeURIComponent(routeId)}/driver-evidence`);box.innerHTML=evidenceBox(x);}catch(e){box.innerHTML=evidenceBox(null);}
}
...
setTimeout(()=>{window.capTpProfiles=profiles;render();refreshProfiles(true)},300);
})();
